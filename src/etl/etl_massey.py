...
def calculate_massey():
    """Calculate the median, mean, and standard deviation of Massey rankings
       for each team per season."""

    # Read in and combine all Massey files
    massey_files = [f for f in os.listdir('./data/kaggle/') if f.startswith('MMassey') or f.startswith('WMassey')]
    ...
    df = df.drop_duplicates(subset=['Season', 'SystemName', 'TeamID'], keep='last')
    df = df.sort_values(by=['Season', 'RankingDayNum', 'SystemName', 'TeamID'], ignore_index=True)
    ...
    df = df.reset_index().rename(columns={'median':'MasseyMedian', 'mean':'MasseyMean', 'std':'MasseyStd'})
