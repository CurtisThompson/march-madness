...
def find_538_ratings(start_year=2016, end_year=2022):
    """Creates list of 538 rankings that can be joined to other features."""

    # Get day after selection sunday (so rankings should be set for tournament)
    ...
    for i in range(start_year, end_year+1):
        ...
        df_year = df_year.sort_values('forecast_date').drop_duplicates(['team_id', 'forecast_date', 'gender'], keep='last')
    ...
    df_m = pd.merge(df_538_m, df_spellings_m, how='left', left_on='team_name', right_on='TeamNameSpelling')
    ...
    df_w = pd.merge(df_538_w, df_spellings_w, how='left', left_on='team_name', right_on='TeamNameSpelling')
